...
def extract_deadlines_llm(text: str) -> List[Dict[str, str]]:
    """
    LLM deadline extractor with future-year policy:
      - If the matched text does NOT include a 4-digit year, interpret it as the NEXT upcoming date >= today.
      - If the model returns a past iso_date for a yearless match, we roll it forward here.
    Returns list of {match, iso_date, time, context}.
    """
    base = datetime.now()
    today_iso = base.date().isoformat()

    # Clear, testable contract for the model
    sys = (
        "You extract deadlines from text. Return JSON exactly as: "
        "{\"deadlines\": [{\"match\": str, \"iso_date\": \"YYYY-MM-DD\", \"time\": \"HH:MM\" or \"\", \"context\": str}, ...]}.\n"
        "Rules:\n"
        "1) If the original matched phrase contains a 4-digit year (e.g., 2026), use that year.\n"
        "2) If it DOES NOT include a year (e.g., \"Feb 7th\", \"by Monday\"), interpret it as the NEXT upcoming date "
        "relative to TODAY.\n"
        "3) TODAY is " + today_iso + ". Your iso_date must be >= TODAY for yearless phrases.\n"
        "4) Keep 'time' empty unless you see an explicit time (e.g., 14:30, 9:00am).\n"
        "5) 'context' should be a short nearby snippet from the input text."
    )

    usr = (
        "Text:\n" + (text or "") + "\n\n"
        "Extract all plausible deadlines following the rules above."
    )

    out = _call_llm_json(sys, usr, model=OPENAI_MODEL_DEFAULT, max_output_tokens=1600)
    items = out.get("deadlines", []) or []

    safe: List[Dict[str, str]] = []
    for it in items:
        match = (it.get("match") or "").strip()
        iso   = (it.get("iso_date") or "").strip()
        tm    = (it.get("time") or "").strip()
        ctx   = (it.get("context") or "").strip()

        # detect if the original match explicitly had a 4-digit year
        has_year = bool(re.search(r"\b\d{4}\b", match))

        # If no year was present in the user's text, ensure future-or-equal date
        # even if the model returned a past year.
        if iso:
            try:
                y, m, d = [int(x) for x in iso.split("-")]
                dt = datetime(y, m, d)
                if not has_year and dt.date() < base.date():
                    dt = _roll_forward_to_future(dt, base)
                    iso = dt.date().isoformat()
            except Exception:
                # If iso is malformed, drop it
                iso = ""
    ...
    # de-duplicate by (iso_date, normalized match)
    uniq, seen = [], set()
    for d in safe:
        key = (d["iso_date"], d["match"].lower())
        if key in seen:
