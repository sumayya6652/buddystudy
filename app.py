    ...
    if "quiz_qs" in st.session_state and st.session_state.quiz_qs:
        ...
        if check:
            if not _all_answered():
                ...
                correct = 0
                for i, q in enumerate(st.session_state.quiz_qs, start=1):
                    your = st.session_state.quiz_answers.get(f"q_{i}")
                    ok = (your == q["answer"])
                    if ok: correct += 1
                    feedback.append({
                        "i": i,
                ...
                total = len(st.session_state.quiz_qs)
                st.success(f"Score: {correct}/{total} ({round(100*correct/total,1)}%)")

                # show per-question feedback right away
                st.subheader("Per-question feedback")
                for fb in st.session_state.quiz_feedback:
                    badge = "✅ Correct" if fb["ok"] else "❌ Incorrect"
                    color = "#2ecc71" if fb["ok"] else "#e74c3c"
                    extra = "" if fb["ok"] else f"<br><b>Correct answer:</b> {fb['answer']}"
                    st.markdown(
                        f"""
                        <div class="card" style="border-left:6px solid {color};">
                        <b>Q{fb['i']}.</b> {fb['question']}<br>
                        <span class="small-muted">Topic: {fb['topic']}</span><br><br>
                        <b style="color:{color};">{badge}</b><br>
                        <b>Your answer:</b> {fb['your']}{extra}
                        </div>
                        """,
                        unsafe_allow_html=True
                    )
